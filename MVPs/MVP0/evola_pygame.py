...
C_TOOLBAR = (32, 32, 48)
C_BTN = (48, 48, 68)
C_BTN_HOVER = (68, 68, 90)
C_BTN_ACTIVE = (90, 140, 90)
C_BTN_DISABLED = (38, 38, 52)
C_TEXT = (220, 220, 220)
C_DIM = (130, 130, 155)
C_PANEL_W = 260
C_TERRAIN = {0: (28, 28, 42), 1: (52, 68, 52), 2: (65, 155, 70),
             3: (225, 60, 50), 4: (217, 162, 58)}
C_VISITED = (75, 75, 95)
C_GRID_LINE = (22, 22, 36)
C_EVOLA = (217, 162, 58)
C_EVOLA_HL = (229, 183, 61)
C_DEBUG = (214, 92, 92)
C_PRIVACY = (94, 91, 107)
C_BAR_BG = (38, 38, 54)
...
class EvolaPygame:
    def __init__(self):
        ...
        self._font = None; self._font_sm = None
        self._update_fonts()
    ...
    def _update_fonts(self):
        ...
        self._font = self._load_font(sz)
        self._font_sm = self._load_font(max(9, sz - 2))

    def _cell_size(self):
        if not self.platform_running or not self.world:
            return 24
        h = self.world.cfg.height
        avail_h = self._height - TOOLBAR_H - STATUS_H
        return max(8, avail_h // h)
    ...
    def cmd_extract(self, agent_id):
        ...
        h = agent.homeo
        data = {"format": "evola.mind", "version": "mvp0.1", "agent_id": agent_id,
                "created_at": datetime.now().isoformat(),
                "saved_step": self.world.step_count, "step_count": agent.step_count,
                "homeostasis": {"energy": h.energy.value,
                                "novelty_visited_count": len(agent.visited),
    ...
    def cmd_stop(self):
        ...
        self.junjian = None; self.bus = None; self.paused = True; self.auto_step = False
        self.events.clear()
    ...
    def _record(self, aid, info):
        s = self.world.step_count; g = info.get("energy_gained", 0)
        d = info.get("damage_taken", 0); c = info.get("collision", False)
        if g > 0: t, ev = "eat", T["ate_food"]
        elif d > 0: t, ev = "danger", T["took_damage"]
        elif c: t, ev = "move", T["hit_wall"]
        elif info.get("food_visible"): t, ev = "explore", T["saw_food"]
        else: t, ev = "explore", T["exploring"]
        self.events.append((s, t, f"{aid} {ev}"))
    ...
    def _draw_btn(self, rect, text, enabled=True, active=False):
        ...
        tpos = (r.centerx - t.get_width()//2, r.centery - t.get_height()//2)
        self._screen.blit(t, tpos)
    ...
    def _draw_panel(self, px, py, w_px):
        cell = self._cell_size()
        world_w = self.world.cfg.width * cell if self.world else 200
        panel_x = world_w + 4
        panel_w = self._width - panel_x - 4
        if panel_w < 160: return
        px2 = panel_x + 6; py2 = TOOLBAR_H + 6
        pw = panel_w - 12
        ...
        if not self.agents:
            t = f.render(T["no_agents"], True, C_DIM)
            self._screen.blit(t, (px2, py2))
            return

        aid = next(iter(self.agents)); ag = self.agents[aid]
        h = ag.homeo; d = h.get_drive()
        nv = len(ag.visited); total = h.novelty.total_cells
        pos = self.world.get_agent_pos(aid)

        t = f.render(f"{aid} @{pos}", True, C_EVOLA)
        self._screen.blit(t, (px2, py2)); py2 += 26

        last_ev = self.events[-1][2] if self.events else T["idle"]
        t = fs.render(last_ev, True, C_EVOLA_HL)
        self._screen.blit(t, (px2, py2)); py2 += 22

        # Energy
        el = _label(h.energy.value, {0.2: "starving", 0.5: "hungry", 0.8: "peckish", 1.0: "full"})
        self._screen.blit(fs.render(f"{T['energy']} ", True, C_DIM), (px2, py2))
        v = f"{h.energy.value:.2f} {el}" if self.debug else el
        self._screen.blit(fs.render(v, True, C_TEXT), (px2 + 50, py2)); py2 += 20
        self._draw_bar(px2, py2, pw, 10, h.energy.value, C_EVOLA); py2 += 16

        # Novelty
        nl = _label(h.novelty.value, {0.3: "familiar", 0.5: "curious", 0.8: "restless", 1.0: "bored"})
        self._screen.blit(fs.render(f"{T['novelty']} ", True, C_DIM), (px2, py2))
        v = f"{h.novelty.value:.2f} {nl}" if self.debug else nl
        self._screen.blit(fs.render(v, True, C_TEXT), (px2 + 50, py2)); py2 += 20
        self._draw_bar(px2, py2, pw, 10, h.novelty.value, C_NOVELTY_BAR); py2 += 16

        # Safety
        sl = _label(h.safety.value, {0.2: "safe", 0.5: "alert", 0.8: "scared", 1.0: "terrified"})
        self._screen.blit(fs.render(f"{T['safety']} ", True, C_DIM), (px2, py2))
        v = f"{h.safety.value:.2f} {sl}" if self.debug else sl
        self._screen.blit(fs.render(v, True, C_TEXT), (px2 + 50, py2)); py2 += 20
        self._draw_bar(px2, py2, pw, 10, 1 - h.safety.value, (230, 65, 55)); py2 += 18
        ...
        self._screen.blit(fs.render(pltxt, True, plc), (px2 + 20, py2)); py2 += 24

        # Events
        log_y = py2 + 6
        self._screen.blit(fs.render(T["events"], True, C_DIM), (px2, log_y)); log_y += 18
        evc = {"eat": C_EVOLA, "danger": (230, 65, 55), "move": C_DIM, "explore": (150, 180, 150)}
        for _, etype, desc in reversed(list(self.events)[-9:]):
            c = evc.get(etype, C_DIM)
            self._screen.blit(fs.render(f"{desc}", True, c), (px2, log_y)); log_y += 16

    def _draw_grid(self):
        if not self.platform_running or not self.world: return
        w = self.world; cell = self._cell_size()
            ...
            for c in range(w.cfg.width):
                ...
                for aid, ag in self.agents.items():
                    if (r, c) == w.get_agent_pos(aid):
                        pygame.draw.circle(self._screen, C_EVOLA,
                                           (c*cell+cell//2, TOOLBAR_H+r*cell+cell//2), max(3, cell//2-1))
                if w.terrain[r, c] == 0 and (r, c) in visited:
                    vr = max(2, cell//6)
                    pygame.draw.circle(self._screen, C_VISITED,
                                       (c*cell+cell//2, TOOLBAR_H+r*cell+cell//2), vr)

    def _draw_toolbar(self):
        world_w = self.world.cfg.width * self._cell_size() if self.world else 400
        total_w = max(self._width, world_w + C_PANEL_W)
        pygame.draw.rect(self._screen, C_TOOLBAR, (0, 0, total_w, TOOLBAR_H))
        self._btn_rects.clear()
        running = self.platform_running
        has_agents = bool(self.agents)

        btns = [
            (T["start"], "start", not running),
            (T["spawn"], "spawn", running and not self.auto_step),
            (T["run100"], "run100", running and not self.auto_step),
            (T["resume"] if (self.auto_step and not self.paused) else T["pause"], "pause", running),
            (T["extract"], "extract", has_agents),
            (T["debug"], "debug", running),
            (T["stop_btn"], "stop", running),
            (T["quit_btn"], "quit", True),
        ]
        x = 4
        for label, cmd, enabled in btns:
            w_btn = max(60, self._font_sm.size(label)[0] + 16)
            r = pygame.Rect(x, 2, w_btn, TOOLBAR_H - 4)
    ...
    def run(self):
        while self.running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        if not self.platform_running or not self.agents:
                            self.running = False
            ...
            self._draw_toolbar()
            self._draw_grid()
            self._draw_panel(0, TOOLBAR_H, self.world.cfg.width * self._cell_size() if self.world else 0)
            self._draw_status()
    ...
    def _handle_cmd(self, cmd):
        ...
        if cmd == "start": self.cmd_start()
            ...
            self.auto_step = True; self.paused = False; self.run_steps = 100
